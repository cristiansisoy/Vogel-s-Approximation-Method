...
public abstract class AbsGrafoD extends AbsGrafo implements OperacionesGD{
	...
	protected ListaDoubleLinkedL listaDistancia, listaCamino, listaSolucion;//para que las uso?

	public AbsGrafoD(int ordenGrafo){
		super(ordenGrafo);
	}
	public AbsGrafoD(int fila, int columna) {
		super(fila, columna);
	}
	...
	public void muestraGrafo(){
		double currCost;
		for (int i=0; i<this.matrizCosto.getNroFilas();i++){	//modificado a conveniencia
			for (int j=0; j<this.matrizCosto.getNroColumnas();j++){
				//if (i!=j){ 	//modificado para que muestre todas las conexiones
	...
	public String muestraGrafoGui(){
		...
		for (int i=0; i<getFila();i++){	//modificado a conveniencia
			for (int j=0; j<getColumna();j++){
				//if (i!=j){ 	//modificado para que muestre todas las conexiones
					currCost=(double)this.matrizCosto.devolver(i, j);
					if (currCost!=infinito){
						cadena += "costo " + i + " a " + j + " -> " + currCost + "\n";
					}				
				//}
			}			
		}
